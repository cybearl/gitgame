...
function skipPropertyExtensions(reader: UAssetBufferReader): void {
    ...
    if ((extensions & CONSTANTS.uasset.propertyTagExtension.overridableInformation) !== 0) {
        reader.uint8() // "OverriddenPropertyOperation"
        reader.bool32() // "ExperimentalOverridableLogic"
    }

    if ((extensions & CONSTANTS.uasset.propertyTagExtension.hasExternalsObjects) !== 0) {
...
function readTagHeader(reader: UAssetBufferReader, names: string[]): UAssetPropertyTag | null {
    ...
    const arrayIndex = (flags & CONSTANTS.uasset.propertyTagFlag.hasArrayIndex) === 0 ? 0 : reader.int32()
    const propertyGuid = (flags & CONSTANTS.uasset.propertyTagFlag.hasPropertyGuid) === 0 ? "" : reader.fguid()

    if ((flags & CONSTANTS.uasset.propertyTagFlag.hasPropertyExtensions) !== 0) {
    ...
    return {
        ...
        size,
        arrayIndex,
        boolValue: (flags & CONSTANTS.uasset.propertyTagFlag.boolTrue) !== 0,
        skipped: (flags & CONSTANTS.uasset.propertyTagFlag.skippedSerialize) !== 0,
        binaryOrNativeSerialize: (flags & CONSTANTS.uasset.propertyTagFlag.hasBinaryOrNativeSerialize) !== 0,
        propertyGuid,
        valueOffset,
    }
}
 ...
 * absolute `valueOffset` and byte `size` so callers can locate the raw payload later, only
 * the modern tag format (post-`PROPERTY_TAG_COMPLETE_TYPE_NAME`, UE 5.4+) is supported.
...
export function readUAssetPropertyTags(
    ...
    if (summary.fileVersionUE5 < UE5.PROPERTY_TAG_COMPLETE_TYPE_NAME.value) {
        throw new Error(
            dedent`Legacy property tag format (pre-PROPERTY_TAG_COMPLETE_TYPE_NAME,
            UE5 version ${summary.fileVersionUE5}) is not supported by this reader.`,
        )
    }

...
export function attachWindowStateBroadcaster(window: BrowserWindow) {
    ...
    const broadcast = () => {
        ...
        window.webContents.send(CONSTANTS.ipc.windowStateChanged, getWindowState(window))
    }
...
export function registerWindowControlHandlers() {
    ipcMain.handle(CONSTANTS.ipc.windowGetState, event => {
        const window = BrowserWindow.fromWebContents(event.sender)
        if (!window) return null
        return getWindowState(window)
    })

    ipcMain.on(CONSTANTS.ipc.windowMinimize, event => {
        BrowserWindow.fromWebContents(event.sender)?.minimize()
    })

    ipcMain.on(CONSTANTS.ipc.windowMaximizeToggle, event => {
        const window = BrowserWindow.fromWebContents(event.sender)
        if (!window) return
    ...
    ipcMain.on(CONSTANTS.ipc.windowClose, event => {
